...
public class LeaderboardRegistry {
    ...
    private static final List<String> COPS_AND_CRIMS_GUNS = Arrays.asList(
        "autoShotgun","bullpup","carbine","handgun","magnum","pistol","rifle","scopedRifle","shotgun","smg","sniper"
    );

    private static final List<String> MEGA_WALLS_CLASSES = Arrays.asList(
        "angel","arcanist","assassin","automaton","blaze","cow","creeper","dreadlord","dragon","enderman","golem","herobrine","hunter","moleman","phoenix","pigman","pirate","renegade","shaman","shark","skeleton","snowman","spider","squid","werewolf","zombie"
    );
    ...
    private static void registerBlitzLeaderboards() {
        ...
        for (String kit : BLITZ_KITS) {
            ...
            new Leaderboard("BLITZ_" + blitzKitUpper + "_EXP", BLITZ,
                    bsg -> JsonUtil.getNullableInt(bsg.get("exp_" + kit)));
            ...
            new Leaderboard("BLITZ_" + blitzKitUpper + "_DAMAGE_DEALT", BLITZ,
                    bsg -> JsonUtil.getNullableInt(bsg.get("damage_" + kit)));
        ...
        new Leaderboard("BLITZ_RANDOM_KILLS", BLITZ, // These have different formatting for some reason???
                bsg -> bsg.get("kills_random").getAsInt());
    ...
    private static void registerNetworkLeaderboards() {
        new Leaderboard("NETWORK_FIRST_LOGIN", NETWORK,
                profile -> profile.get("first_login").getAsLong(), 1);
        new Leaderboard("NETWORK_NETWORK_LEVEL", NETWORK,
                profile -> profile.get("network_level").getAsFloat());
        new Leaderboard("NETWORK_ACHIEVEMENT_POINTS", NETWORK,
                profile -> profile.get("achievement_points").getAsInt());
        new Leaderboard("NETWORK_KARMA", NETWORK,
                profile -> profile.get("karma").getAsInt());
    ...
    private static void registerBedWarsLeaderboards() {
        // General stats
        new Leaderboard("BEDWARS_EXP", BEDWARS, bw -> bw.get("Experience").getAsLong());
    ...
    private static void registerPitLeaderboards() {
        ...
        new Leaderboard("PIT_EXP", PIT, pit -> pit.getAsJsonObject("profile").get("xp").getAsLong());
    ...
    private static void registerBuildBattleLeaderboards() {
        ...
        new Leaderboard("BUILD_BATTLE_SCORE", BUILD_BATTLE, bb -> bb.get("score").getAsInt());
    ...
    private static void registerMurderMysteryLeaderboards() {
        new Leaderboard("MURDER_MYSTERY_KILLS", MURDER_MYSTERY, mm -> mm.get("kills").getAsInt());
        new Leaderboard("MURDER_MYSTERY_WINS", MURDER_MYSTERY, mm -> mm.get("wins").getAsInt());
    ...
    private static void registerTNTGamesLeaderboards() {
        new Leaderboard("TNT_GAMES_WINS", TNT_GAMES, tnt -> tnt.get("wins").getAsInt());
    ...
    private static void registerArenaBrawlLeaderboards() {
        ...
        addToLookup(new Leaderboard("ARENA_BRAWL_WINS", ARENA_BRAWL, ab -> ab.get("wins").getAsInt()));
    ...
    private static void registerVampireZLeaderboards() {
        ...
        new Leaderboard("VAMPIREZ_HUMAN_WINS", VAMPIREZ, vz -> vz.get("human_wins").getAsInt());
        new Leaderboard("VAMPIREZ_HUMAN_KILLS", VAMPIREZ, vz -> vz.get("human_kills").getAsInt());
    ...
    private static void registerCopsAndCrimsLeaderboards() {
        new Leaderboard("COPS_AND_CRIMS_SCORE", COPS_AND_CRIMS, cc -> cc.get("score").getAsInt());
    ...
    private static void registerMegaWallsLeaderboards() {
        ...
        for (String cls : MEGA_WALLS_CLASSES) {
            ...
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_CLASS_POINTS", MEGA_WALLS, mw -> mw.get(cls + "_class_points").getAsInt());
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_WINS", MEGA_WALLS, mw -> mw.get(cls + "_wins").getAsInt());
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_FINAL_KILLS", MEGA_WALLS, mw -> mw.get(cls + "_final_kills").getAsInt());
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_FINAL_ASSISTS", MEGA_WALLS, mw -> mw.get(cls + "_final_assists").getAsInt());
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_DEFENDER_KILLS", MEGA_WALLS, mw -> mw.get(cls + "_defender_kills").getAsInt());
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_PLAYTIME", MEGA_WALLS, mw -> mw.get(cls + "time_played").getAsInt());
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_WITHER_DAMAGE", MEGA_WALLS, mw -> mw.get(cls + "_wither_damage").getAsInt());
            new Leaderboard("MEGA_WALLS_" + clsUpper + "_DAMAGE_DEALT", MEGA_WALLS, mw -> mw.get(cls + "_damage_dealt").getAsInt());
        }
    }
    ...
    private static void registerUHCLeaderboards() {
        ...
        new Leaderboard("UHC_SCORE", UHC, uhc -> uhc.get("score").getAsInt());
    ...
    private static void registerSpeedUHCLeaderboards() {
        new Leaderboard("SPEED_UHC_SCORE", SPEED_UHC, suhc -> suhc.get("score").getAsInt());
...
new Leaderboard("WOOL_GAMES_LEVEL", WOOL_GAMES, wg -> wg.getAsJsonObject("progression").get("experience").getAsInt());

addToLookup(new Leaderboard("WOOL_GAMES_SHEEP_WARS_WINS", WOOL_GAMES, wg -> wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("wins").getAsInt()));
new Leaderboard("WOOL_GAMES_SHEEP_WARS_KDR", WOOL_GAMES, wg -> wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("kills").getAsDouble() / Math.max(wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("deaths").getAsDouble(), 1));
addToLookup(new Leaderboard("WOOL_GAMES_SHEEP_WARS_KILLS", WOOL_GAMES, wg -> wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("kills").getAsInt()));
new Leaderboard("WOOL_GAMES_SHEEP_WARS_WLR", WOOL_GAMES, wg -> wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("wins").getAsDouble() / Math.max(wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("losses").getAsDouble(), 1));

addToLookup(new Leaderboard("WOOL_GAMES_WOOL_WARS_WINS", WOOL_GAMES, wg -> wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("wins").getAsInt()));
new Leaderboard("WOOL_GAMES_WOOL_WARS_KDR", WOOL_GAMES, wg -> wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("kills").getAsDouble() / Math.max(wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("deaths").getAsDouble(), 1));
addToLookup(new Leaderboard("WOOL_GAMES_WOOL_WARS_KILLS", WOOL_GAMES, wg -> wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("kills").getAsInt()));
new Leaderboard("WOOL_GAMES_WOOL_WARS_WLR", WOOL_GAMES, wg -> wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("wins").getAsDouble() / Math.max(JsonUtil.getNullableInt(wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("games_played")) - JsonUtil.getNullableInt(wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("wins")), 1));

addToLookup(new Leaderboard("WOOL_GAMES_CAPTURE_THE_WOOL_WINS", WOOL_GAMES, wg -> wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("participated_wins").getAsInt()));
new Leaderboard("WOOL_GAMES_CAPTURE_THE_WOOL_KDR", WOOL_GAMES, wg -> wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("kills").getAsDouble() / Math.max(wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("deaths").getAsDouble(), 1));
addToLookup(new Leaderboard("WOOL_GAMES_CAPTURE_THE_WOOL_KILLS", WOOL_GAMES, wg -> wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("kills").getAsInt()));
new Leaderboard("WOOL_GAMES_CAPTURE_THE_WOOL_WLR", WOOL_GAMES, wg -> wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("participated_wins").getAsDouble() / Math.max(wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("participated_losses").getAsDouble(), 1));

addToLookup(new Leaderboard("WOOL_GAMES_WINS", WOOL_GAMES, wg -> LEADERBOARDS.get("WOOL_GAMES_SHEEP_WARS_WINS").derive(wg).intValue() + LEADERBOARDS.get("WOOL_GAMES_WOOL_WARS_WINS").derive(wg).intValue() + LEADERBOARDS.get("WOOL_GAMES_CAPTURE_THE_WOOL_WINS").derive(wg).intValue()));
addToLookup(new Leaderboard("WOOL_GAMES_KILLS", WOOL_GAMES, wg -> LEADERBOARDS.get("WOOL_GAMES_SHEEP_WARS_KILLS").derive(wg).intValue() + LEADERBOARDS.get("WOOL_GAMES_WOOL_WARS_KILLS").derive(wg).intValue() + LEADERBOARDS.get("WOOL_GAMES_CAPTURE_THE_WOOL_KILLS").derive(wg).intValue()));

new Leaderboard("WOOL_GAMES_WLR", WOOL_GAMES, wg -> LEADERBOARDS.get("WOOL_GAMES_WINS").derive(wg).doubleValue() / Math.max(1, JsonUtil.getNullableInt(wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("games_played")) - JsonUtil.getNullableInt(wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("wins")) + JsonUtil.getNullableInt(wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("losses")) + JsonUtil.getNullableInt(wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("participated_losses"))));

new Leaderboard("WOOL_GAMES_KDR", WOOL_GAMES, wg -> LEADERBOARDS.get("WOOL_GAMES_KILLS").derive(wg).doubleValue() / Math.max(1, JsonUtil.getNullableDouble(wg.getAsJsonObject("wool_wars").getAsJsonObject("stats").get("deaths")) + JsonUtil.getNullableDouble(wg.getAsJsonObject("sheep_wars").getAsJsonObject("stats").get("deaths")) + JsonUtil.getNullableDouble(wg.getAsJsonObject("capture_the_wool").getAsJsonObject("stats").get("deaths"))));
    }
    ...
    private static void registerFishingLeaderboards() {
        ...
        addToLookup(new Leaderboard("FISHING_FISH_CAUGHT", FISHING, fish -> LEADERBOARDS.get("FISHING_WATER_FISH_CAUGHT").derive(fish).intValue()
                + LEADERBOARDS.get("FISHING_LAVA_FISH_CAUGHT").derive(fish).intValue() + LEADERBOARDS.get("FISHING_ICE_FISH_CAUGHT").derive(fish).intValue()));
        addToLookup(new Leaderboard("FISHING_TREASURE_CAUGHT", FISHING,
                fish -> LEADERBOARDS.get("FISHING_WATER_TREASURE_CAUGHT").derive(fish).intValue()
                        + LEADERBOARDS.get("FISHING_LAVA_TREASURE_CAUGHT").derive(fish).intValue()
                        + LEADERBOARDS.get("FISHING_ICE_TREASURE_CAUGHT").derive(fish).intValue()));
        addToLookup(new Leaderboard("FISHING_JUNK_CAUGHT", FISHING, fish -> LEADERBOARDS.get("FISHING_WATER_JUNK_CAUGHT").derive(fish).intValue()
                + LEADERBOARDS.get("FISHING_LAVA_JUNK_CAUGHT").derive(fish).intValue() + LEADERBOARDS.get("FISHING_ICE_JUNK_CAUGHT").derive(fish).intValue()));
        ...
        addToLookup(new Leaderboard("FISHING_TOTAL_CAUGHT", FISHING,
                fish -> LEADERBOARDS.get("FISHING_FISH_CAUGHT").derive(fish).intValue() + LEADERBOARDS.get("FISHING_TREASURE_CAUGHT").derive(fish).intValue()
