	...
	public JsonObject build(@NonNull String name) {
        ...
        */
		...
		if (hypixelStats != null) { // Null if the player has no stats (never logged in)
			...
			// Some staff members have their API disabled
			if (hypixelStats.has("achievements")) {
				response.add("achievements", hypixelStats.get("achievements").getAsJsonObject());
			} else {
				response.add("achievements", new JsonObject()); // Fallback if stats are off/missing
			}

			if (hypixelStats.has("achievementsOneTime")) {
				response.add("achievementsOneTime", hypixelStats.get("achievementsOneTime").getAsJsonObject());
			} else {
