...
package io.nadeshiko.nadeshiko.achievements;

import com.google.gson.JsonObject;
import io.nadeshiko.nadeshiko.api.AchievementsController;
 ...
 * <p>
 *
 * The /achievements endpoint controller ({@link AchievementsController}) utilizes the {@link AchievementsCache#get(String)} method to fetch
 * the API response for a given player. If the response is not in the cache, it relies upon the {@link AchievementsBuilder}
 * instance to build a new response, which is then cached and returned.
 *
 * @see AchievementsBuilder
 * @author chloe
 */
public class AchievementsCache extends Cache<String, AchievementsCache.CacheEntry> {

	/**
	 * The Builder instance used to generate responses
	 * @see AchievementsBuilder
	 */
	private final AchievementsBuilder builder = new AchievementsBuilder();
	...
	/**
	 * An entry within the cache, mapped to in {@link AchievementsCache#cache} using player names as keys. Stores the time
